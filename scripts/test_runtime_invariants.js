...
function assertSourcePatternGuards() {
  ...
  assert(
    /roleFate = mapTrajectoryToCompatibilityFate\(/.test(source),
    'Compatibility fate labels should be derived from the canonical trajectory state.'
  );
  assert(
    /compatibility_exports:\s*{[\s\S]*?wave_trajectory:\s*compatibilityWaveTrajectory/.test(source),
    'Wave-shaped summaries should be quarantined under compatibility_exports.'
  );
  assert(
...
async function main() {
  ...
  occupations.forEach((occupation) => {
    ...
    const compatibilityNext = result.compatibility_exports?.wave_trajectory?.next || {};
