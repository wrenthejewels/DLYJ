...
async function main() {
  ...
  for (const row of EXPECTATIONS) {
    ...
    const compatibilityFate = result.compatibility_exports?.role_fate_label;
    if (compatibilityFate !== row.expected) {
      throw new Error(`${row.title} expected ${row.expected} but received ${compatibilityFate}.`);
    }
    ...
    results.push({
      ...
      confidence: result.compatibility_exports?.role_fate_confidence,
