...
async function main() {
  ...
  if (!result.timing_frontier.primary_displacement_wave) {
    throw new Error('Expected timing_frontier.primary_displacement_wave in result payload.');
  }
  ...
  if (!result.compatibility_exports?.wave_trajectory) {
    throw new Error('Expected wave_trajectory only inside compatibility_exports.');
  }
  ['current', 'next', 'distant'].forEach((wave) => {
    const snapshot = result.compatibility_exports.wave_trajectory[wave];
    if (!snapshot) {
      throw new Error(`Expected compatibility_exports.wave_trajectory.${wave} in result payload.`);
    }
    assertBounded(`compatibility_exports.wave_trajectory.${wave}.retained_share`, snapshot.retained_share);
    assertBounded(`compatibility_exports.wave_trajectory.${wave}.coherence`, snapshot.coherence);
  });

  if (Object.prototype.hasOwnProperty.call(result, 'primary_displacement_wave')) {
    throw new Error('Expected primary_displacement_wave to live under timing_frontier, not top-level result.');
  }
  ...
  assertBounded('compatibility_exports.primary_displacement_wave_confidence', result.compatibility_exports.primary_displacement_wave_confidence);
  ...
  if (!result.state_trajectory?.headline) {
    throw new Error('Expected state_trajectory.headline in result payload.');
  }
  if (Object.prototype.hasOwnProperty.call(result, 'role_fate_label')) {
    throw new Error('Expected role_fate_label to live under compatibility_exports, not top-level result.');
  }
  ['accountability_context', 'bargaining_power_context', 'fragmentation_context', 'accountability_context_confidence', 'bargaining_context_confidence', 'fragmentation_context_confidence'].forEach((key) => {
  ...
  assertBounded('taskDrivenResult.compatibility_exports.role_fate_confidence', taskDrivenResult.compatibility_exports.role_fate_confidence);
  ...
  assertBounded('structuredProfileResult.compatibility_exports.role_fate_confidence', structuredProfileResult.compatibility_exports.role_fate_confidence);
  ...
  console.log(JSON.stringify({
    summary: {
      ...
      roleOutlook: result.compatibility_exports?.role_outlook_label,
      stateHeadline: result.state_trajectory?.headline,
      primaryDisplacementWave: result.timing_frontier?.primary_displacement_wave,
    ...
    waveTrajectory: {
      current: result.state_trajectory.checkpoints.current.state + ' (' + (1 - result.state_trajectory.checkpoints.current.transformed_share).toFixed(3) + ')',
      next: result.state_trajectory.checkpoints.next.state + ' (' + (1 - result.state_trajectory.checkpoints.next.transformed_share).toFixed(3) + ')',
      distant: result.state_trajectory.checkpoints.distant.state + ' (' + (1 - result.state_trajectory.checkpoints.distant.transformed_share).toFixed(3) + ')'
    },
    counts: {
    ...
    taskSelection: {
      ...
      stateHeadline: taskDrivenResult.state_trajectory?.headline,
    ...
    structuredProfile: {
      stateHeadline: structuredProfileResult.state_trajectory?.headline,
