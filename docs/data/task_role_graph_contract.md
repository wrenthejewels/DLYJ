...
- `1785` task rows in `data/normalized/occupation_task_inventory.csv`
- `64` modeled occupations currently have task-inventory rows
- average `27.9` rows per covered occupation
- minimum `22`
- maximum `38`
...
- the reviewed job-description layer now carries `392` rows: `32` occupations at `4` reviewed rows, `30` at `8`, and `2` at `12`
