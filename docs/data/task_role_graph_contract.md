...
- the same scored task rows now also feed the exported `trajectory.timeline` graph path, so the client can plot a continuous baseline `L(t)` line, a conservative/aggressive envelope band, a max-`dP/dt` inflection marker, and baseline `P(t)` threshold crossings from the same live task graph
