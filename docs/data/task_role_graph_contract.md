...
- the same scored task rows now also feed the exported `trajectory.timeline` graph path, so the client can plot a continuous transformed-share curve from `P(t)`, a conservative/aggressive envelope band, a max-`dP/dt` inflection marker, and the `30%`, `50%`, and `70%` baseline `P(t)` crossings from the same live task graph
