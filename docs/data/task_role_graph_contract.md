...
- `1745` task rows in `data/normalized/occupation_task_inventory.csv`
- `64` modeled occupations currently have task-inventory rows
- average `27.3` rows per covered occupation
- minimum `19`
- maximum `38`
...
- the reviewed job-description layer now carries `352` rows: `41` occupations at `4` reviewed rows, `22` at `8`, and `1` at `12`
