...
- `1549` task rows in `data/normalized/occupation_task_inventory.csv`
- `64` modeled occupations currently have task-inventory rows
- average `24.2` rows per covered occupation
- minimum `11`
- maximum `35`
...
- `Receptionists and Information Clerks` (`18`)
...
- targeted manual review expansions now cover `39` occupations, so the newly promoted occupations still remain the main review queue
