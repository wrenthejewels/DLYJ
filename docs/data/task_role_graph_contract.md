...
- `1805` task rows in `data/normalized/occupation_task_inventory.csv`
- `64` modeled occupations currently have task-inventory rows
- average `28.2` rows per covered occupation
- minimum `23`
- maximum `38`
...
- the reviewed job-description layer now carries `412` rows: `27` occupations at `4` reviewed rows, `35` at `8`, and `2` at `12`
