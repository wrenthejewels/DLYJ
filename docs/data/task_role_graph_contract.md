...
- `1589` task rows in `data/normalized/occupation_task_inventory.csv`
- `64` modeled occupations currently have task-inventory rows
- average `24.8` rows per covered occupation
- minimum `15`
- maximum `35`
...
- `Computer User Support Specialists` (`20`)
...
- targeted manual review expansions now cover `49` occupations, so the newly promoted occupations still remain the main review queue
