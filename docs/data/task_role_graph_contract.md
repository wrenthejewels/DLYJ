...
- the reviewed function graph now carries `114` occupation-function rows across `63` occupations, and `49` occupations now start from more than one reviewed default function anchor
...
- the remaining gap is not "no graph" or "no anchor depth" for the promoted cohort; it is that evidence density and multi-anchor maturity are still uneven across the library, and `Business Operations Specialists, All Other` remains the one selected occupation without direct GPT task-label coverage from that source
