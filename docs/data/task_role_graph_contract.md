...
- `1845` task rows in `data/normalized/occupation_task_inventory.csv`
- `64` modeled occupations currently have task-inventory rows
- average `28.8` rows per covered occupation
- minimum `24`
- maximum `38`
...
- the reviewed job-description layer now carries `452` rows: `24` occupations at `4` reviewed rows, `32` at `8`, `7` at `12`, and `1` at `16`
