...
- the live result now also exposes a `trajectory.timeline` block so the client can render a role-level graph from a continuous time process: one dense baseline `L(t)` line, a conservative/aggressive envelope band, a max-`dP/dt` inflection marker, and threshold markers placed at the baseline `P(t)` crossing years
