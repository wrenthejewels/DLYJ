...
- `state_trajectory` is now the primary interpretation layer beside the canonical continuous `trajectory` contract
- `trajectory` still carries the compression/demand/structural-necessity math; `state_trajectory` turns those signals into current, next, and long-run structural states plus checkpoint/timeline views
- top-level `role_fate_*`, `role_outlook`, `wave_trajectory`, and `primary_displacement_wave` exports were removed from the main result surface
- older wave/fate-shaped fields now live only inside `compatibility_exports` for external callers that have not migrated
- `compatibility_exports.wave_trajectory` is derived from `state_trajectory.checkpoints`, not from a separate wave engine
- `timing_frontier.primary_displacement_wave` is the canonical seat-break timing bucket, chosen from the earlier of the `compress` and `structural_break` hurdle crossings
- the score-per-fate diagnostic classifier has been removed from the runtime and payload; transition-trigger and seat-change summaries now use the trajectory-derived compatibility projection only for wording
- the task-role graph now also gets its cluster retained-share / elevation context from the same shared cluster-frontier helper before graph diagnostics run, so task-graph retained-share diagnostics and exported cluster timing rows no longer depend on different prepasses- the latest calibration pass now lets the trajectory layer read reviewed function-category structure when estimating demand response and structural necessity, so revenue-facing, coordination-heavy, and governance-heavy roles no longer share one flat Jevons/retention path
...
- the canonical user-facing classifier is the structural state trajectory
- compatibility fate labels are deterministic projections from `trajectory.state` and `trajectory.role_shape`
- compatibility labels are retained for older integrations but are not a second classifier and are not described in the public methodology as the model's headline
...
- `Execution is leaving this role. Judgment is what stays.` is reserved for roles with a coherent retained core and stronger retained human leverage
...
- the canonical trajectory classifier reads distant-scenario viability decline and distant pressure-demand balance, so `stable`, `transforming`, `compressing`, and `collapsing` can all fire under the shipped runtime instead of the earlier first-pass `expanding/unsettled` concentration
...
- `Technical Writers`, `Editors`, `Management Analysts`, `Accountants and Auditors`, and `Office Clerks, General` now use explicit reviewed role variants, with sharper reviewed task/function weighting inside those variant baselines
...
- the latest reviewed-anchor accountability pass also sharpens several live defaults:
  - `Computer User Support Specialists` now uses lighter reviewed `support_enablement` and `workflow_adoption_enablement` guardrails
  - `Loan Interviewers and Clerks` now uses lighter reviewed `lending_intake_execution` and `intake_exception_coordination` guardrails
  - `General and Operations Managers`, `Human Resources Managers`, and `Transportation, Storage, and Distribution Managers` now use stronger owner-heavy manager primaries or secondary anchors
  - `Computer Systems Analysts`, `Financial and Investment Analysts`, and `Mechanical Engineers` now use slightly lighter reviewed advisory/analysis anchors
- the promoted `next 30` occupation cohort now also has reviewed default multi-anchor coverage end to end: all `30` promoted occupations start from two reviewed default function anchors, and a large reviewed subset now also overrides the role-family default primary anchor where the generic family primary was materially misleading
- representative promoted-cohort corrections now include `Financial Managers` separating executive financial direction from resource-allocation leadership, `Sales Managers` separating team-level commercial direction from flatter revenue-creation defaults, `Computer User Support Specialists` separating frontline support enablement from workflow-adoption work, `Personal Financial Advisors` separating advice from relationship-book stewardship, and `Court, Municipal, and License Clerks` separating public-record administration from case-window coordination
...
- the latest kept structural-pressure calibration also lets a bounded share of `cluster_drafting`, `cluster_documentation`, and `cluster_research_synthesis` contribute to `routine_high_pressure_share` when that work is already under meaningful direct pressure in lower-people, knowledge-heavy roles; this is a pressure-accounting correction, not a claim that those clusters are literally clerical
...
- `compatibility_exports.wave_trajectory` now comes from compatibility snapshots derived from `state_trajectory.checkpoints`, while `timing_frontier.primary_displacement_wave` comes from the shared timing frontier
...
- the live engine now derives compatibility wave summaries from state checkpoints rather than preserving a separate pre-task wave bundle
...
- `timing_frontier.primary_wave_score` is now a continuous timing score built from trigger readiness and scenario activation; the score itself is no longer floored or capped by the categorical timing bucket
...
- the frontier's `primary_binding_constraint` now comes from whichever displacement trigger (`compress` or `structural_break`) actually sets the earliest crossing wave, instead of defaulting to compression-only blockers
