...
- reviewed supplemental anchors now extend beyond the explicit role-variant subset across the full selected library; every selected occupation now exposes more than one reviewed default function anchor even without a separate variant selector
...
- `occupation_individual_ai_usage_context.csv` is now a narrow runtime input for the trajectory layer's present-day transformation floor where observed individual AI usage exists; it does not affect task automability, task-level evidence resolution, or outer demand/adoption context
- `occupation_recomposition_context.csv` is now a runtime input for the outer recomposition/timing layer; it does not affect task automability, but it does constrain workflow compression, organizational conversion, scenario activation, and timing-frontier hurdle crossings through fields such as `next_scenario_lift`, `distant_scenario_lift`, `organizational_adoption_ceiling`, and `economic_pressure_context`
