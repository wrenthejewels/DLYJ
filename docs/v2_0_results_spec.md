...
- the same top-level state forecast still uses the client-side `STATE_FORECAST_WEIGHTS` mapping, but the five-state mix is now surfaced through a hero-chart inspector rather than through a second stacked support chart
