...
Current note:
 the trigger layer now does a better job separating straight-compression roles from roles that genuinely rebundle around a clearer retained core, and the repo now includes a default-map regression snapshot plus a compact audit export for those outputs. A follow-up function-depth pass also resolved the earlier docs/runtime drift for the support and clerical queue and closed the last true one-anchor occupations. Trigger confidence reasons are now also more specific: most roles read as task-backed and context-aligned, while the weaker cases are separated into adjacent-threshold ties, crowded ordering, or thin rebundle evidence instead of collapsing into one generic line. The latest contract-hardening pass also makes the earliest future structural shift explicit as `state_trajectory.first_transition_state` / `first_transition_year`, keeps `likely_next_state` only as a compatibility alias, clarifies that the highlighted decisive trigger is the seat-level turning threshold rather than the highest-readiness row, hardens the seat map so growing bundles must carry positive net share deltas, and renames the seat-share fields as overlapping diagnostics instead of one additive pie while the top retained-core strip now reads the current checkpoint directly.
