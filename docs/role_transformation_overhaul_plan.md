...
- a quarantined compatibility-export block for older API callers
- a checkpoint-derived compatibility wave projection inside `compatibility_exports`
...
- `state_trajectory` and `timing_frontier` are now the live model surfaces described by the methodology
- top-level wave/fate aliases were removed from the main result payload; older callers must read `compatibility_exports`
- `timing_frontier.primary_displacement_wave` remains as the canonical seat-break timing bucket, chosen from the earlier `compress` vs `structural_break` hurdle crossing
- `compatibility_exports.wave_trajectory` is derived from `state_trajectory.checkpoints`, not from a separate wave engine
- the score-per-fate diagnostic classifier was removed from the runtime after it proved less stable than the continuous trajectory/state layer
...
- the reviewed calibration target file was refreshed to the current recomposition scale, and `evaluate_v2_calibration.js --update-targets --strict` now regenerates those ranges and the calibration report intentionally rather than requiring ad hoc CSV edits
...
- the client no longer treats wave/fate-shaped fields as active runtime state; compatibility fields are quarantined under `compatibility_exports`
- the transition-trigger layer reads shared checkpoint diagnostics rather than exported wave-shaped objects
...
- the structural-state trajectory is the live classifier
- older fate-shaped labels are deterministic projections inside `compatibility_exports`; they are retained for older callers, not used as a parallel model
- the trajectory classifier is tuned against the default occupation-map regression snapshot and reads distant-scenario viability decline as well as next-scenario levels so `stable`, `transforming`, `compressing`, and `collapsing` are all reachable in the shipped runtime
...
- known issues documented for future work:
  - **advanced-detail overlap**: the structural state model is now the primary results surface, while some trajectory detail still renders as supporting explanation. Keep this secondary material visibly subordinate to the state read.
  - **external compatibility cleanup**: `compatibility_exports` still preserves older wave/fate-shaped fields for integrations. Remove it only after external consumers have migrated to `state_trajectory` and `timing_frontier`.
...
- ~~**Role fate decision tree brittleness**~~ *(resolved)* — the separate fate classifier was removed from the live runtime. The canonical model now uses `state_trajectory` plus `timing_frontier`; old fate-shaped labels are deterministic compatibility projections under `compatibility_exports`.
...
- a later reviewed-anchor accountability pass then did the narrower mixed cleanup the queue still justified: it softened reviewed support/intake guardrails for `Computer User Support Specialists` and `Loan Interviewers and Clerks`, strengthened owner-heavy manager anchors for `General and Operations Managers`, `Human Resources Managers`, and `Transportation, Storage, and Distribution Managers`, and softened overstated advisory/sign-off ownership for `Computer Systems Analysts`, `Financial and Investment Analysts`, and `Mechanical Engineers`; after that pass the strong ORS-backed human-guardrail check improved again to about `0.903`
...
- a follow-up calibration pass also lowered the task-cluster adoption-realization intercept from `0.92` to `0.84` while keeping the `0.16` slope, so default-profile runs no longer behave like near-fully realized adoption before occupation-level context is applied
