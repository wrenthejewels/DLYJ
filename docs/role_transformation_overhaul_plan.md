...
- the promoted cohort's baseline function-depth pass is also now complete: all `30` promoted occupations have reviewed two-anchor default function graphs, and the latest proxy-edge cap plus authored-edge sufficiency passes removed the worst fanout and filled the clearest missing reviewed links, so the next debt is targeted variant/anchor maturity and occupation-by-occupation graph nuance rather than blank function coverage or broad proxy pruning
