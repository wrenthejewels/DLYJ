...
The next occupation-level pass reintroduced the older structural scatter, but only after translating it into the new model language. The occupation block paired the full dominant-state matrix with that restored structural diagnostic map. The important constraint was to keep the old axes where they were still useful while stripping out fate labels and wave-first language that no longer matched the live model. The restored map now behaves like a secondary structural diagnostic rather than a competing headline forecast.
