...
The next occupation-landscape runtime pass addressed a performance failure rather than a scoring one. The new matrix/comparison views and the restored diagnostic map had each been computing the full launch set independently, which made the section visibly slow on desktop and fragile on smaller devices. The occupation-level surfaces now share one cached snapshot under the active assumption controls, and the diagnostic map initializes lazily instead of competing with the rest of the page during first paint.

The next occupation-level pass reintroduced the older structural scatter, but only after translating it into the new model language. The occupation block paired the full dominant-state matrix with that restored diagnostic map. The important constraint was to keep the old axes where they were still useful while stripping out fate labels and wave-first language that no longer matched the live model. The restored map now behaves like a secondary structural diagnostic rather than a competing headline forecast, and it now follows the same comparison hierarchy and slider assumptions as the occupation table above.
...
The next calibration pass focused only on timing. By then the structural report was clearly saying that the weakest shared layer was no longer accountability or pressure; it was timing. The reason was simple: the calibration target was continuous, but the live `primary_wave_score` still inherited most of its shape from a coarse compatibility bucket (`current`, `next`, `distant`). That meant two occupations with very different readiness and activation profiles could still collapse to the same timing score. The fix kept the old wave label for backward compatibility but replaced the score underneath it. `primary_wave_score` is now a continuous blend of compress readiness, structural-break readiness, delegate readiness, assist readiness, and scenario-activation lift. A later audit then removed the residual within-wave floor/cap that was still collapsing all current-wave roles to one shared score, so the continuous timing layer now preserves within-bucket spread instead of only renaming the same categorical rank. That pulled timing back into line with the rest of the model without changing the public state taxonomy.

The same audit also cleaned up the task-evidence resolver. Source reliability and `evidence_weight` had drifted into each other, so some curated source rows were effectively getting weighted twice. The runtime now keeps reliability as an epistemic confidence term and applies `evidence_weight` only once when building the task-level consensus. At the same time, the timing frontier stopped imposing hidden `0.5` floors on retained bargaining and retained accountability, so weak-retention roles can now express genuinely weak retained leverage inside compression and structural-break timing instead of inheriting an undocumented optimism floor.

The next follow-up finished demoting the old role-level wave layer. The engine still exports `current`, `next`, and `distant` compatibility snapshots, but those snapshots are now derived from `state_trajectory.checkpoints` instead of being maintained as a separate forecasting path. Retained share now comes straight from the checkpoint complement of transformed share, coherence comes from checkpoint role integrity, and the old compatibility state labels are thresholded off those same checkpoint states. That matters because it removes one more duplicated interpretation layer: continuous trajectory/state outputs now own the role-level forecast, while waves survive only as compatibility summaries and timing-frontier shorthand.

The next step finished the more important half of that port: timing itself. Before this pass, the exported compatibility wave object had been demoted, but the timing frontier still took retained-share and integrity inputs from the old role-wave pre-pass and only later exported state-derived compatibility summaries. The runtime now pushes timing onto the same continuous foundation as the public state layer. `timing_frontier` reads the next checkpoint from `state_trajectory`, and `primary_displacement_wave` is chosen from the earlier of the `compress` and `structural_break` crossings instead of from a compression-first override with special-case promotion. That closes the biggest remaining gap between what the docs said and what the engine actually did.

One more cleanup pass then removed the last hidden hybrid seam inside the role graph. Even after the timing frontier moved to continuous checkpoints, the task-role graph was still computing retained-share diagnostics from an earlier cluster prepass than the one used for the exported cluster bundle. The runtime now seeds the graph from the same shared cluster-frontier enrichment helper, then runs the task-derived cluster bundle back through that helper for the final export. That is a smaller change than the timing-frontier port, but it matters for rigor: retained-share diagnostics, cluster timing labels, and exported cluster summaries now all come from the same continuous cluster-frontier path instead of from two subtly different ones.

The next cleanup was less about math than about language discipline inside the runtime. By then the public timing and state layers were already continuous-first, but a few downstream consumers still talked to them through wave-shaped aliases. The live fate classifier now reads the next checkpoint state and the continuous frontier score before any compatibility wave fields, the occupation-landscape snapshot now exports checkpoint metrics under checkpoint names instead of `current_wave_*` / `next_wave_*` aliases, and the calibration script now starts from `timing_frontier.primary_wave_score` rather than from `primary_displacement_wave`. Waves still ship, but they are now much closer to what they were supposed to become: a thin compatibility/presentation layer on top of the continuous engine rather than an internal working language.

The next structural follow-up was intentionally narrower than the earlier shared-function retune. A broader structural rewrite briefly improved some queue counts, but it degraded the stronger accountability and bargaining checks too much to keep. The surviving change was the one that held up under the next full sweep: codifiable drafting, documentation, and research-synthesis work now contributes a bounded share into the structural pressure path when that work is already under meaningful direct pressure in lower-people, knowledge-heavy roles. That means content-heavy roles no longer read as structurally untouched just because they are not clerical. The kept pass lifted the routine-pressure audit again while leaving the stronger human-guardrail check intact and only modestly softening the weak wage-leverage proxy.

The next accountability pass returned to the reviewed function layer instead of trying another broad formula rewrite. By then the strong ORS-backed queue was mixed: some frontline support and intake roles still carried too much durable ownership, while several owner-heavy manager roles still looked slightly flatter than their ORS context suggested. The fix stayed local to the reviewed anchor rows. `Computer User Support Specialists` and `Loan Interviewers and Clerks` got lighter support/intake guardrails. `General and Operations Managers`, `Human Resources Managers`, and `Transportation, Storage, and Distribution Managers` got stronger owner-heavy manager anchors. `Computer Systems Analysts`, `Financial and Investment Analysts`, and `Mechanical Engineers` got slightly lighter advisory/analysis guardrails so expert judgment stayed intact without reading like formal sign-off ownership. That pass pushed the strongest human-guardrail check up again into the low-0.90s without another blunt shared-formula tradeoff.
...
- the secondary stacked state-share chart was later restored as a smaller support panel beneath the five-year read, while the hero tooltip kept the lighter quick-read detail
- a later calibration pass tightened the client-side displaced-share mapping after review showed it was reading too aggressive too early. The fix added an explicit transition-pressure term, a state-aware gate keyed to the engine's current structural state, and a separate integrity cutoff instead of reusing the transformed-share floor. The hero displacement headline was also changed to follow the engine tipping-point layer rather than a raw displaced-share threshold.
- the next reconciliation pass found that the standalone score-per-fate diagnostic had become too optimistic as the continuous state layer matured. The runtime removed that separate classifier entirely. The live result now treats `state_trajectory` and `timing_frontier` as the model, with older wave/fate-shaped fields quarantined under `compatibility_exports` for external callers only. The default-map and timing regression snapshots now track structural-state distributions, and the methodology no longer asks readers to understand a parallel fate model.
